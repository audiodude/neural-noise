"""A copy of the produce script, with tweaks for nn flavored output.

Should be combined/refactored with the other script.
"""
...
RE_NOTE = re.compile(r'[A-G]')

def get_generated_data(temperature, cp_path):
  cmdline = ('th sample.lua cv/%(file)s -primetext "#" -length %(length)s '
...
"""
  ...
  chords = ''
  def extract_chord(md):
    global chords
    if md.group(1):
      chords += ' ' + RE_NOTE.sub(r'\g<0>,', md.group(1))
    return ''
  notes = RE_CHORD.sub(extract_chord, song)
  notes = notes.replace('|', '').replace('#', '')

  return '%(meta)s[V:notes] %(notes)s\n[V:chords]%(chords)s\n' % {
    'meta': meta,
    'notes': notes,
    'chords': chords
  }
...
def insert_generated_songs(temperature, cp_path):
  count = 0
  for generated in get_generated_songs(temperature, cp_path):
    song = reconstruct_song(generated)
    count += 1
